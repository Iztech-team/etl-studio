...
def read_columns(table: str) -> List[str]:
    ...
    try:
        with p.open("r", encoding="utf-8", errors="replace", newline="") as f:
            ...
            row = next(reader, [])
            return [
                c for c in row if c and c.replace("_", "").replace("-", "").isalnum()
            ]
    except Exception:
        return []


def renamed_with_drops(table: str, rename: Dict[str, tuple]) -> List[Dict[str, Any]]:
...
def build_pos_profile() -> List[Dict[str, Any]]:
    ...
    edits += [
        fk_lookup("warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
        fixed("company", DEFAULT_COMPANY),
        fixed("disabled", "0", "integer"),
        fixed("currency", "NIS"),
...
def build_item_default_from_categoryt() -> List[Dict[str, Any]]:
    ...
    edits += [
        from_col("name", "parent"),
        fk_lookup(
            "default_warehouse", "STORET", "DESCRIPTION", "STOREID", "DEFAULTSTOREID"
        ),
        fk_lookup("income_account", "CATBASICSETST", "SALEACC", "SETID", "SETNO"),
        fk_lookup("expense_account", "CATBASICSETST", "PURCHACC", "SETID", "SETNO"),
        fk_lookup(
            "default_inventory_account", "CATBASICSETST", "STORAGEACC", "SETID", "SETNO"
        ),
        fixed("parenttype", "Item"),
...
def build_item_price() -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        edit(
            ...
            "integer",
            op="rename",
            target_name="price_list_legacy_id",
            target_type="integer",
        ),
        edit(
            "price_list",
...
def build_customer() -> List[Dict[str, Any]]:
    ...
    edits += [
        fk_lookup("customer_name", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNT"),
...
def build_supplier() -> List[Dict[str, Any]]:
    ...
    edits += [
        fk_lookup("supplier_name", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNT"),
        fk_lookup_chain(
            "default_currency",
            "ACCOUNT",
            chain=[
...
def build_employee() -> List[Dict[str, Any]]:
    ...
    edits += [
        fk_lookup("employee_name", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNT"),
...
def build_account() -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        fk_lookup("parent_account", "ACCOUNTT", "ACCOUNTID", "ACCOUNTID", "FATHERID"),
        fk_lookup("account_currency", "CURT", "CURSHORT", "CURID", "CURID"),
...
def _sales_invoice_header(table: str, *, is_return: bool) -> List[Dict[str, Any]]:
    ...
    edits += [
        edit(
            ...
            is_new=True,
            generator={"kind": "fixed", "value": ""},
            transforms=[{"op": "concat_template", "params": {"template": template}}],
        ),
        fk_lookup("customer", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNTID"),
        fk_lookup("set_warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
...
def _sales_invoice_item(table: str, *, is_return: bool) -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        fk_lookup("warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
        fk_lookup("income_account", "ACCOUNTT", "NAME", "ACCOUNTID", "SALEACCID"),
...
def _purchase_invoice_header(table: str, *, is_return: bool) -> List[Dict[str, Any]]:
    ...
    for col in cols:
        if col in rename:
            ...
            edits.append(edit(col, op="drop"))
    template = "AL-PRET-{docno_legacy}" if is_return else "AL-PINV-{docno_legacy}"
    edits += [
        edit(
            "name",
            src_type="string",
        ...
        fk_lookup("supplier", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNTID"),
        fk_lookup("set_warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
        fk_lookup("currency", "CURT", "CURSHORT", "CURID", "CURID"),
        fixed("status", "Submitted"),
        fixed("docstatus", "1", "integer"),
...
def _purchase_invoice_item(table: str, *, is_return: bool) -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        from_col("idx", "idx_legacy"),
        fk_lookup("warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
        fk_lookup("expense_account", "ACCOUNTT", "NAME", "ACCOUNTID", expense_field),
        computed("qty", qty_expr),
        computed("amount", "{qty} * {rate}"),
        computed("net_amount", "{qty} * {rate} - {discount_amount}"),
...
def build_stock_entry_internal() -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        fk_lookup("from_warehouse", "STORET", "DESCRIPTION", "STOREID", "FROMSTOREID"),
        fk_lookup("to_warehouse", "STORET", "DESCRIPTION", "STOREID", "TOSTOREID"),
...
def _stock_entry_detail(table: str, parent_prefix: str) -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        from_col("idx", "idx_legacy"),
        fk_lookup("s_warehouse", "STORET", "DESCRIPTION", "STOREID", "STOREID"),
        fk_lookup("t_warehouse", "STORET", "DESCRIPTION", "STOREID", "TOSTOREID"),
...
def _journal_entry_header(
    table: str, voucher_type: str, prefix: str
) -> List[Dict[str, Any]]:
    edits = renamed_with_drops(
        table,
        {
...
def _journal_entry_account(table: str, prefix: str) -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        from_col("idx", "idx_legacy"),
        fk_lookup("account", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNTID"),
        fk_lookup_chain(
            "account_currency",
            "ACCOUNTID",
            chain=[
...
def _payment_doc_header(
    ...
    edits += [
        ...
        fixed("party_type", "Customer" if payment_type == "Receive" else "Supplier"),
        fixed("docstatus", "1", "integer"),
        fixed("company", DEFAULT_COMPANY),
        fixed("owner", DEFAULT_OWNER),
    ]
...
def build_cheques_in_flight() -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        fk_lookup("party", "ACCOUNTT", "NAME", "ACCOUNTID", "ACCOUNTID"),
        fixed("payment_type", "Receive"),
        fixed("party_type", "Customer"),
...
def build_gl_entry() -> List[Dict[str, Any]]:
    ...
    edits += [
        fk_lookup("account", "ACCOUNTT", "NAME", "ACCOUNTID", "ENTRYACCOUNT"),
        fk_lookup("account_currency", "CURT", "CURSHORT", "CURID", "ENTRYCUR"),
        fixed("is_opening", "No"),
...
def build_stock_ledger_entry() -> List[Dict[str, Any]]:
    ...
    edits += [
        ...
        computed("actual_qty", "{actual_qty_in} - {actual_qty_out}", round_=4),
        fixed("is_cancelled", "0", "integer"),
        fixed("docstatus", "1", "integer"),
        fixed("company", DEFAULT_COMPANY),
        fixed("owner", DEFAULT_OWNER),
...
def build() -> Dict[str, Any]:
    ...
    table_options["CUSTT"] = {
        "row_filter": {
            "mode": "drop",
            "conditions": [
                {"column": "customer_name", "op": "eq", "value": WALK_IN_NAME}
            ],
        },
    }
    ...
    extra_configs.append(
        {
            ...
            "row_filter": {
                "mode": "drop",
                "conditions": [
                    {"column": "MANUFACTURER", "op": "is_null", "value": None}
                ],
            },
        }
    )
